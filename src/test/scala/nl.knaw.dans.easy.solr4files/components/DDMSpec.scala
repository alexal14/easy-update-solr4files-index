...
class DDMSpec extends TestSupportFixture with FileReaderComponent {
  ...
  "solrLiteral" should "return proper values" in {
    ...
    val xml = vault.fileURL("pdbs", uuid, "metadata/dataset.xml").flatMap(reader.loadXml).getOrElse(<ddm/>)
